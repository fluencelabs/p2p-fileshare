...
import Element exposing (Element, centerX, column, el, fillPortion, mouseOver, padding, paddingXY, paragraph, pointer, row, spacing, text, width)
...
addFileBlock screenInfo model =
    let
        ...
        emptyColumn = Element.el [ width <| fillPortion 4 ] Element.none

        intro = el [paddingXY 0 10, F.size5] <| paragraph [] [text "Add file from your device or enter IPFS hash"]
        or = el [paddingXY 0 10, F.size5] <| text "OR"
    ...
    in
        if (phonePortrait screenInfo) then
